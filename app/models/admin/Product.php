...
class Product extends AppModel{
    ...
    public function getImg(){    //   virezat     //
    ...
    public function uploadImg($name, $wmax, $hmax){
        $uploaddir = WWW . '/images/';//'/uploaded_files'
        $ext = strtolower(preg_replace("#.+\.([a-z]+)$#i", "$1", $_FILES[$name]['name'])); //расширение картинки
        $types = array("image/gif", "image/png", "image/jpeg", "image/pjpeg", "image/x-png"); // массив допустимый расширений
        ...
        if (!in_array($_FILES[$name]['type'], $types)) {
            $res = array("error" => "Допустимые расшинеия- .gif, .jpg, .png");
            exit(json_encode($res));
        }
    ...
    /**
     * @param string $target путь к оригиналному файлу
    ...
    public static function resize($target, $dest, $wmax, $hmax, $ext){
        ...
        $ratio = $w_orig / $h_orig; // =1-квадрат, <1-албомная, >1-книжная
        if (($wmax / $hmax) > $ratio) {
            $wmax = $hmax * $ratio;
        } else {
            $hmax = $wmax / $ratio;
        }
        $img = "";    // imagecreatefromjpeg | imagecreatefromgif |imagecreatefrompng
        switch ($ext) {
            case ("gif"):
        ...
        $newImg = imagecreatetruecolor($wmax, $hmax); // создаем оболочку для новый картинки
        if ($ext == "png") {
        ...
        imagecopyresampled($newImg, $img, 0, 0, 0, 0, $wmax, $hmax, $w_orig, $h_orig);//копируем и резаизим изображения
        switch ($ext) {
            case ("gif"):
